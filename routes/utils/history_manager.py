...
class HistoryManager:
    ...
    def _ensure_database_exists(self):
        ...
        with self._get_connection() as conn:
            ...
            self._ensure_table_schema(
                cursor,
                "download_history",
                expected_download_history_columns,
                "download history",
            )

            # 2.5) Backfill defaults for critical columns to avoid NULLs post-migration
            try:
                ...
                if "title" in cols:
                    cursor.execute(
						...
						SET title = COALESCE(NULLIF(title, ''), 'Unknown')
                ...
                if "download_type" in cols:
                    cursor.execute(
                        """
						UPDATE download_history
						SET download_type = COALESCE(NULLIF(download_type, ''), 'unknown')
						WHERE download_type IS NULL OR download_type = ''
						"""
                    )
            except Exception as e:
                logger.warning(
                    f"Non-fatal: failed to backfill defaults for download_history: {e}"
                )
            ...
            try:
                ...
                if "timestamp" not in cols:
                    # Add column first
                    cursor.execute(
                        "ALTER TABLE download_history ADD COLUMN timestamp REAL"
                    )
                    # Backfill from legacy columns if present
                    legacy_time_cols = [
                        c for c in ["time", "created_at", "date"] if c in cols
                    ]
                    if legacy_time_cols:
                        ...
                        try:
                            cursor.execute(
                                f"UPDATE download_history SET timestamp = CASE WHEN {legacy_col} IS NOT NULL THEN {legacy_col} ELSE strftime('%s','now') END"
                            )
                        except sqlite3.Error:
                            # Fallback: just set to now
                            cursor.execute(
                                "UPDATE download_history SET timestamp = strftime('%s','now')"
                            )
                    else:
                ...
                if "quality_format" not in cols and "quality" in cols:
                    cursor.execute(
                        "ALTER TABLE download_history ADD COLUMN quality_format TEXT"
                    )
                    try:
                        cursor.execute(
                ...
                if "quality_bitrate" not in cols and "bitrate" in cols:
                    cursor.execute(
                        "ALTER TABLE download_history ADD COLUMN quality_bitrate TEXT"
                    )
                    try:
                        cursor.execute(
                ...
                logger.warning(
                    f"Non-fatal: failed legacy column migration for download_history: {e}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Non-fatal: failed to create indexes for download_history: {e}"
                )
        ...
        try:
            ...
            for col_name, col_type in expected_columns.items():
                if col_name not in existing_names:
                    # Avoid adding PRIMARY KEY or NOT NULL on existing tables; strip them for ALTER
                    col_type_for_add = (
    ...
    def create_children_table_for_playlist(
        self, callback_data: Dict, task_id: str
    ) -> str:
        """
        ...
        logger.info(
            f"Created playlist children table {children_table} for task {task_id}"
        )
        return children_table
    ...
    def store_track_history(
        ...
        try:
            ...
            # Prefer service/quality/bitrate from summary when available
            summary = status_info.get("summary") or {}
            service = summary.get("service") or self._get_primary_service(external_ids)
            quality_format = summary.get("quality") or status_info.get("convert_to")
            quality_bitrate = summary.get("bitrate") or status_info.get("bitrate")
            ...
            with self._get_connection() as conn:
                if table == "download_history":
                    ...
                    logger.info(
                        f"Storing track '{track.get('title', 'Unknown')}' in MAIN table for task {task_id}"
                    )
                    conn.execute(
                        """
                    ...
                    logger.info(
                        f"Storing track '{track.get('title', 'Unknown')}' in CHILDREN table '{table}' for task {task_id}"
                    )
                    # Extract ISRC
            ...
            logger.info(
                f"Successfully stored track '{track.get('title')}' in table '{table}' (task: {task_id})"
            )

        except Exception as e:
            logger.error(f"Failed to store track history for task {task_id}: {e}")

    def store_album_history(
        self, callback_data: Dict, task_id: str, status: str = "completed"
    ):
        """
        ...
        try:
            ...
            if status == "in_progress":
                ...
                children_table = self.create_children_table_for_album(
                    callback_data, task_id
                )
                logger.info(
                    f"Album download started for task {task_id}, children table: {children_table}"
                )
                return children_table
            ...
            if not children_table:
                ...
                children_table = f"album_{album_uuid}"
                logger.warning(
                    f"No children table found for album task {task_id}, generating new: {children_table}"
                )
            ...
            total_tracks = summary.get("total_successful", 0) + summary.get(
                "total_skipped", 0
            ) + summary.get("total_failed", 0) or album.get("total_tracks", 0)
            ...
            with self._get_connection() as conn:
                ...
                # Prefer service/quality/bitrate from summary when available
                service_to_store = summary.get("service") or self._get_primary_service(
                    external_ids
                )
                quality_format_to_store = summary.get("quality") or status_info.get(
                    "convert_to"
                )
                quality_bitrate_to_store = summary.get("bitrate") or status_info.get(
                    "bitrate"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to populate children from summary for album {children_table}: {e}"
                )

            logger.info(
                f"Stored album history for '{album.get('title')}' (task: {task_id}, children: {children_table}, status: {status_to_store})"
            )
            return None
    ...
    def store_playlist_history(
        ...
        try:
            ...
            if status == "in_progress":
                ...
                children_table = self.create_children_table_for_playlist(
                    callback_data, task_id
                )
                logger.info(
                    f"Playlist download started for task {task_id}, children table: {children_table}"
                )
                return children_table
            ...
            if not children_table:
                ...
                children_table = f"playlist_{playlist_uuid}"
                logger.warning(
                    f"No children table found for playlist task {task_id}, generating new: {children_table}"
                )
            ...
            total_tracks = (
                summary.get("total_successful", 0)
            ...
            with self._get_connection() as conn:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        json.dumps([owner.get("name", "Unknown")]),
                        callback_data.get("timestamp", time.time()),
                        status_to_store,
                        service_to_store,
                        quality_format_to_store,
            ...
            logger.info(
                f"Stored playlist history for '{playlist.get('title')}' (task: {task_id}, children: {children_table}, status: {status_to_store})"
            )
            return None
    ...
    def _populate_album_children_table(
        ...
        try:
            ...
            logger.info(
                f"Populated {len(all_rows)} tracks in children table {table_name}"
            )
    ...
    def _populate_playlist_children_table(
        ...
        try:
            ...
            logger.error(
                f"Failed to populate playlist children table {table_name}: {e}"
            )
    ...
    def _prepare_child_track_data(
        ...
        # Prepare metadata
        metadata = {"album": album, "type": track.get("type", "")}
    ...
    def get_children_history(self, children_table: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Failed to retrieve children history from {children_table}: {e}"
            )
            return []
            ...
            with self._get_connection() as conn:
                ...
                for table_name in children_tables:
                    try:
                        ...
                        logger.warning(
                            f"Failed to drop children table {table_name}: {e}"
                        )

                logger.info(
                    f"Cleared {deleted_count} old history records and {len(children_tables)} children tables"
                )
    ...
    def _populate_failed_children_for_album(
        ...
        try:
            ...
            logger.error(
                f"Failed to insert failed children for album into {table_name}: {e}"
            )
    ...
    def _populate_failed_children_for_playlist(
        ...
        try:
            ...
            logger.error(
                f"Failed to insert failed children for playlist into {table_name}: {e}"
            )
    ...
    def _enrich_album_metadata_from_summary(self, album: Dict, summary: Dict) -> Dict:
        ...
        for key in ("successful_tracks", "skipped_tracks", "failed_tracks"):
            ...
            if arr:
                src_track = (
                    arr[0]
                    if key != "failed_tracks"
                    else (arr[0].get("track") if isinstance(arr[0], dict) else None)
                )
                break
        if isinstance(src_track, dict):
            ...
            album.setdefault(
                "album_type", album_obj.get("album_type", album.get("album_type"))
            )
        return album
    ...
    def _merge_child_rows(self, existing: Dict, new: Dict) -> Dict:
        ...
        if self._status_priority(new.get("status")) > self._status_priority(
            existing.get("status")
        ):
        ...
        merged["timestamp"] = max(
            existing.get("timestamp") or 0, new.get("timestamp") or 0
        )
        return merged

    def _find_existing_child_row(
        self, conn: sqlite3.Connection, table_name: str, new_row: Dict
    ) -> Optional[Dict]:
        try:
            cursor = conn.execute(
                f"SELECT * FROM {table_name} WHERE title = ?",
                (new_row.get("title", ""),),
            )
    ...
    def _upsert_child_row(
        self, conn: sqlite3.Connection, table_name: str, row: Dict
    ) -> None:
